...
async function signup(req, res) {
  ...
  if (await isExistUserEmailInDB(email)) {
    ...
    // 필요시 이메일 검증 추가
    let newUser = createUser(email, encryptPassword, nickname, allowNotice, isSuperUser, newSalt);
    try {
      await newUser.save();
      console.log(newUser);
      res.redirect("/login");
 ...
 * @returns {User}
 */
function createUser(email, encryptPassword, nickname, allowNotice, isSuperUser, salt) {
